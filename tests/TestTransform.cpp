...
TEST(TestTransforms, TestGood)
{
    /* //Initialize a PowerWAF rule*/
    //const char * jsonFile = readFile("transform_good.json");
    //ASSERT_TRUE(jsonFile != nullptr);
    //ASSERT_TRUE(pw_init("rule", jsonFile, NULL, NULL));
    //free((void*) jsonFile);

    ////Setup the parameter structure
    //ddwaf_object parameter, ba, map = DDWAF_OBJECT_MAP;
    //ddwaf_object has_pattern = ddwaf_object_string("string to match");
    //ddwaf_object has_uppercase = ddwaf_object_string("String To Match");
    //ddwaf_object has_uppercase_in_key = ddwaf_object_string("nothing to match");
    //ddwaf_object has_null = ddwaf_object_string("_string to match");
    //ddwaf_object has_uppercase_and_null = ddwaf_object_string("_String To Match");

    //((char*) has_null.stringValue)[0] = '\0';
    //((char*) has_uppercase_and_null.stringValue)[0] = '\0';

    //has_uppercase_in_key.parameterName = "String To Match";
    //has_uppercase_in_key.parameterNameLength = strlen(has_uppercase_in_key.parameterName);
    //map.nbEntries = 1;
    //map.array = &has_uppercase_in_key;

    //parameter.nbEntries = 1;
    //parameter.parameterName = NULL;
    //parameter.type = DDWAF_OBJ_MAP;
    //parameter.array = &ba;

    //ba.nbEntries = 1;
    //ba.parameterName = "value";
    //ba.parameterNameLength = strlen(ba.parameterName);
    ...
    //free((void*) has_uppercase.stringValue);
    //free((void*) has_null.stringValue);
    //free((void*) has_uppercase_and_null.stringValue);
    //free((void*) has_uppercase_in_key.stringValue);
...
TEST(TestURLTransforms, DISABLED_Functionnal)
{
    /* ddwaf_handle handle = pw_initH(R"({"manifest": {"arg_base": {"inherit_from": "arg","run_on_value": true,"run_on_key": false,"processor":[{"transforms":["_sqr_basename"]}]}, "arg_file": {"inherit_from": "arg","run_on_value": true,"run_on_key": false,"processor":[{"transforms":["_sqr_filename"]}]}, "arg_query": {"inherit_from": "arg","run_on_value": true,"run_on_key": false,"processor":[{"transforms":["_sqr_querystring"]}]}},"rules": [{"rule_id": "1","filters": [{"operator": "@rx","targets": ["arg_base", "arg_file", "arg_query"],"value": ".*","options":{"multi_match":true}}]}],"flows": [{"name": "test","steps": [{"id": "start","rule_ids": ["1"],"on_match": "exit_block"}]}]})", NULL, NULL);*/
    //ASSERT_NE(handle, nullptr);

    //ddwaf_object map = DDWAF_OBJECT_MAP;
    //pw_addMap(&map, "arg", 0, ddwaf_object_string("/bla/path.php?a=b"));
    ...
    //TEST(TestURLTransforms, DISABLED_Evil)
    //{
    //ddwaf_handle handle = pw_initH(R"({"manifest": {"arg_base": {"inherit_from": "arg","run_on_value": true,"run_on_key": false,"processor":[{"transforms":["_sqr_basename"]}]}, "arg_file": {"inherit_from": "arg","run_on_value": true,"run_on_key": false,"processor":[{"transforms":["_sqr_filename"]}]}, "arg_query": {"inherit_from": "arg","run_on_value": true,"run_on_key": false,"processor":[{"transforms":["_sqr_querystring"]}]}},"rules": [{"rule_id": "1","filters": [{"operator": "@rx","targets": ["arg_base", "arg_file", "arg_query"],"value": ".*","options":{"multi_match":true}}]}],"flows": [{"name": "test","steps": [{"id": "start","rule_ids": ["1"],"on_match": "exit_block"}]}]})", NULL, NULL);
    //ASSERT_NE(handle, nullptr);
    ...
    //{
    //ddwaf_object map = DDWAF_OBJECT_MAP;
    //pw_addMap(&map, "arg", 0, ddwaf_object_string("?a=b"));
    ...
    //{
    //ddwaf_object map = DDWAF_OBJECT_MAP;
    //pw_addMap(&map, "arg", 0, ddwaf_object_string("/bla"));
    ...
    //{
    //ddwaf_object map = DDWAF_OBJECT_MAP;
    //pw_addMap(&map, "arg", 0, ddwaf_object_string("bla.php"));
