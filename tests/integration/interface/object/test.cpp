...
TEST(TestObjectIntegration, TestAddArray)
{
    ...
    EXPECT_TRUE(ddwaf_object_array_add(&container, ddwaf_object_set_signed(&object, 42)));
    EXPECT_TRUE(ddwaf_object_array_add(&container, ddwaf_object_set_unsigned(&object, 43)));
...
TEST(TestObjectIntegration, TestAddMap)
{
    ...
    EXPECT_FALSE(ddwaf_object_map_add(nullptr, "key", ddwaf_object_set_signed(&tmp, 42)));
    ddwaf_object_free(&tmp);
    EXPECT_FALSE(ddwaf_object_map_add(&array, "key", ddwaf_object_set_signed(&tmp, 42)));
    ddwaf_object_free(&tmp);
    EXPECT_FALSE(ddwaf_object_map_add(&map, nullptr, ddwaf_object_set_signed(&tmp, 42)));
    ddwaf_object_free(&tmp);

    EXPECT_FALSE(ddwaf_object_map_add(&map, "key", nullptr));

    EXPECT_TRUE(ddwaf_object_map_add(&map, "key", ddwaf_object_set_invalid(&tmp)));
    EXPECT_STRV(object_to_view(map.via.map.ptr[0].key), "key");

    ASSERT_TRUE(ddwaf_object_map_add(&map, "key", ddwaf_object_set_signed(&tmp, 42)));
    EXPECT_STRV(object_to_view(map.via.map.ptr[1].key), "key");

    ASSERT_TRUE(ddwaf_object_map_addl(&map, "key2", 4, ddwaf_object_set_signed(&tmp, 43)));
    ...
    memcpy(str, "key3", 4);
    ASSERT_TRUE(ddwaf_object_map_addl_nc(&map, str, 4, ddwaf_object_set_signed(&tmp, 44)));
...
TEST(TestObjectIntegration, FindInvalidKey)
{
    ...
    ddwaf_object map;
    ddwaf_object_map(&map);
    ddwaf_object_map_add(&map, "key", ddwaf_object_set_invalid(&tmp));
...
TEST(TestObjectIntegration, FindNotMap)
{
    ...
    ddwaf_object array;
    ddwaf_object_array(&array);
    ddwaf_object_array_add(&array, ddwaf_object_set_invalid(&tmp));
...
TEST(TestObjectIntegration, FindEmptyMap)
{
    ...
    ddwaf_object map;
    ddwaf_object_map(&map);
    ddwaf_object_map_add(&map, "key", ddwaf_object_set_unsigned(&tmp, 42));
