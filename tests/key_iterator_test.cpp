...
TEST(TestKeyIterator, TestContainerMixPath)
{
    ...
    std::pmr::unordered_set<const ddwaf_object *> exclude;
    {
        ...
        auto it_path = it.get_current_path();
        std::pmr::vector<std::pmr::string> path = {"root", "key0", "2", "key0_0"};
...
TEST(TestKeyIterator, TestMapDepthLimitPath)
{
...

    {
        ...
        {
            auto it_path = it.get_current_path();
            std::pmr::vector<std::pmr::string> path = {"root", "child", "grandchild", "unknown"};
            EXPECT_EQ(it_path, path);
        }
        ...
        {
            auto it_path = it.get_current_path();
            std::pmr::vector<std::pmr::string> path = {"root", "child", "grandchild", "another"};
            EXPECT_EQ(it_path, path);
        }
...
TEST(TestKeyIterator, TestExcludeMultipleObjects)
{
    ...
    std::pmr::unordered_set<const ddwaf_object *> exclude = {&root.array[0], &map.array[1]};
    ddwaf::object::key_iterator it(&root, {}, exclude);
