...
TEST(TestUserRuleParser, ParseRule)
{
    ...
    EXPECT_EQ(change.content, change_set::custom_rules);
    EXPECT_EQ(change.user_rules.size(), 1);
    EXPECT_TRUE(change.user_rules.contains("1"));

    EXPECT_EQ(cfg.user_rules.size(), 1);
    EXPECT_TRUE(cfg.user_rules.contains("1"));

    const rule_spec &rule = cfg.user_rules["1"];
...
TEST(TestUserRuleParser, ParseOutputRule)
{
    configuration_spec cfg;
    configuration_change_spec change;
    configuration_collector collector{change, cfg};
...
TEST(TestUserRuleParser, ParseComboRule)
{
    configuration_spec cfg;
    configuration_change_spec change;
    configuration_collector collector{change, cfg};
...
TEST(TestUserRuleParser, ParseMultipleRules)
{
    ...
    EXPECT_FALSE(change.empty());
    EXPECT_EQ(change.content, change_set::custom_rules);
    EXPECT_EQ(change.user_rules.size(), 2);
    EXPECT_TRUE(change.user_rules.contains("1"));
    EXPECT_TRUE(change.user_rules.contains("secondrule"));

    EXPECT_EQ(cfg.user_rules.size(), 2);
    EXPECT_TRUE(cfg.user_rules.contains("1"));
    EXPECT_TRUE(cfg.user_rules.contains("secondrule"));
    ...
    {
        const rule_spec &rule = cfg.user_rules["secondrule"];
        EXPECT_TRUE(rule.enabled);
        EXPECT_EQ(rule.expr->size(), 1);
...
TEST(TestUserRuleParser, InconsequentialRule)
{
    configuration_spec cfg;
    configuration_change_spec change;
    configuration_collector collector{change, cfg};
...
TEST(TestUserRuleParser, InvalidOutputType)
{
    configuration_spec cfg;
    configuration_change_spec change;
    configuration_collector collector{change, cfg};
