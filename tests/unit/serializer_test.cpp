...
TEST(TestEventSerializer, ExtendedDataCollectionAction)
{
    ddwaf::action_mapper_builder builder;
    ...
    std::unordered_map<std::string, std::string> tags{{"type", "test"}, {"category", "none"}};
    std::vector<std::string> actions{"extended_data_collection"};
    std::vector<rule_attribute> attributes;

    rule_result result{
        .event = rule_event{.rule{
                                .id = "edc001",
                                .name = "extended data collection rule",
                                .tags = tags,
                            },
            .matches = {{.args = {{.name = "input",
                             .resolved = "sensitive_value"sv,
                             .address = "headers",
                             .key_path = {"authorization"}}},
                .highlights = {"sensitive"sv},
                .operator_name = "match_regex",
                .operator_value = "sensitive.*"}}},
        .action_override = {},
        .actions = actions,
        .attributes = attributes,
    };
    ...
    EXPECT_EVENTS(result_object, {.id = "edc001",
                                     .name = "extended data collection rule",
                                     .tags = {{"type", "test"}, {"category", "none"}},
                                     .actions = {"extended_data_collection"},
                                     .matches = {{.op = "match_regex",
                                         .op_value = "sensitive.*",
                                         .highlight = "sensitive"sv,
                                         .args = {{.name = "input",
                                             .value = "sensitive_value"sv,
                                             .address = "headers",
                                             .path = {"authorization"}}}}}});

    EXPECT_ACTIONS(result_object,
        {{"extended_data_collection", {{"headers_redaction", "true"}, {"max_collected_headers", "10"}}}});
