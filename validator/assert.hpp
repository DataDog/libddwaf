...
public:
    ...
    template <typename T> assert_exception(const T &lhs, const T &rhs, int loc, std::string_view fn)
    {
        std::stringstream ss;
        ss << fn << "(" << loc << "): " << lhs << " != " << rhs;
        what_ = ss.str();
...
template <> inline void assert(const bool &lhs, const bool &rhs, int loc, std::string_view fn)
{
    if (lhs != rhs) {
...
template <>
inline void assert(
    const DDWAF_RET_CODE &lhs, const DDWAF_RET_CODE &rhs, int loc, std::string_view fn)
{
    if (lhs != rhs) {
        throw assert_exception(to_string(lhs), to_string(rhs), loc, fn);
    }
...
template <typename T>
inline void assert_yaml(const YAML::Node &lhs, const YAML::Node &rhs, int loc, std::string_view fn)
{
...
template <>
inline void assert<YAML::Node>(
    const YAML::Node &lhs, const YAML::Node &rhs, int loc, std::string_view fn)
{
    switch (lhs.Type()) {
