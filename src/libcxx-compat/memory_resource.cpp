...
class __new_delete_memory_resource_imp : public memory_resource {
    ...
    {
        std::__libcpp_deallocate(static_cast<std::byte *>(p), bytes, align);
    }
...
constinit ResourceInitHelper res_init [[gnu::init_priority(101)]];
...
{
    // deallocating a block that was not allocated with this allocator
    assert(__first_ != nullptr);
    if (__first_->__start_ == p) {
        ...
        // The request to deallocate memory ends up being a no-op, likely resulting in a memory
        // leak.
        //
        // deallocating a block that was not allocated with this allocator
        assert(false);
...
public:
    ...
    void __release_ptr(memory_resource *upstream)
    {
        ...
        while (__first_chunk_ != nullptr) {
            ...
            upstream->deallocate(__first_chunk_->__start_, __first_chunk_->__allocation_size(),
                __first_chunk_->__align_);
            __first_chunk_ = next;
        }
    }
...
void unsynchronized_pool_resource::release()
{
    ...
    if (__fixed_pools_ != nullptr) {
        ...
        for (int i = 0; i < n; ++i) __fixed_pools_[i].__release_ptr(__res_);
        __res_->deallocate(
            __fixed_pools_, __num_fixed_pools_ * sizeof(__fixed_pool), alignof(__fixed_pool));
...
void *unsynchronized_pool_resource::do_allocate(size_t bytes, size_t align)
{
    ...
    if (i == __num_fixed_pools_)
        ...
        if (result == nullptr) {
            ...
            size_t prev_chunk_size_in_blocks =
                prev_chunk_size_in_bytes >> __log2_pool_block_size(i);

            size_t chunk_size_in_blocks;

            if (prev_chunk_size_in_blocks == 0) {
                size_t min_blocks_per_chunk =
                    max(__min_bytes_per_chunk >> __log2_pool_block_size(i), __min_blocks_per_chunk);
                chunk_size_in_blocks = min_blocks_per_chunk;
            } else {
                static_assert(__max_bytes_per_chunk <= SIZE_MAX - (__max_bytes_per_chunk / 4),
                    "unsigned overflow is possible");
                chunk_size_in_blocks = prev_chunk_size_in_blocks + (prev_chunk_size_in_blocks / 4);
            }

            size_t max_blocks_per_chunk = min((__max_bytes_per_chunk >> __log2_pool_block_size(i)),
            ...
            result =
                __fixed_pools_[i].__allocate_in_new_chunk(__res_, block_size, chunk_size_in_bytes);
        }
        return result;
    }
}
...
void unsynchronized_pool_resource::do_deallocate(void *p, size_t bytes, size_t align)
{
    ...
    if (i == __num_fixed_pools_)
        ...
        // deallocating a block that was not allocated with this allocator
        assert(__fixed_pools_ != nullptr);
        __fixed_pools_[i].__evacuate(p);
    }
}

bool synchronized_pool_resource::do_is_equal(const memory_resource &other) const noexcept
{
    return &other == this;
}
...
void *monotonic_buffer_resource::do_allocate(size_t bytes, size_t align)
{
    ...
    if (void *result =
            __try_allocate_from_chunk<true, __initial_descriptor>(__initial_, bytes, align))
        return result;
    if (__chunks_ != nullptr) {
        if (void *result =
                __try_allocate_from_chunk<false, __chunk_footer>(*__chunks_, bytes, align))
            return result;
    }
