...
public:
    ...
    exclusion_filter(std::vector<condition::ptr> &&conditions, std::set<rule_base::ptr> &&rule_targets)
        : conditions_(std::move(conditions)), rule_targets_(std::move(rule_targets))
    {}

    [[nodiscard]] const std::set<rule_base::ptr> &get_rule_targets() const { return rule_targets_; }
...
protected:
    ...
    std::set<rule_base::ptr> rule_targets_;
};
