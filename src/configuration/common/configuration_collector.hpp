...
public:
    ...
    void emplace_rule(std::string id, rule_spec spec) {
        if (spec.source == core_rule::source_type::base) {
            ...
            config_.content = config_.content | change_set::rules;
            ...
            config_.content = config_.content | change_set::custom_rules;
    ...
    void emplace_override(std::string id, override_spec spec) {
        ...
        config_.content = config_.content | change_set::overrides;
    ...
    void emplace_filter(std::string id, rule_filter_spec spec) {
        ...
        config_.content = config_.content | change_set::filters;
    ...
    void emplace_processor(std::string id, processor_spec spec) {
        ...
        config_.content = config_.content | change_set::processors;
    ...
    void emplace_scanner(std::shared_ptr<scanner> scanner) {
        ...
        config_.content = config_.content | change_set::scanners;
    ...
    void emplace_action(std::string id, action_spec spec) {
        ...
        config_.content = config_.content | change_set::actions;
    ...
    void emplace_rule_data(std::string data_id, std::string id, data_spec spec) {
        change_.content = change_.content | change_set::rule_data;
        config_.content = config_.content | change_set::rule_data;

        auto it = config_.rule_data.find(data_id);
        if (it == config_.rule_data.end()) {
            auto &merged_spec = config_.rule_data[data_id];
            merged_spec.type = spec.type;
            merged_spec.values.emplace(id, std::move(spec.values));
        } else {
            // TODO fail if type differs
            it->second.values.emplace(id, std::move(spec.values));
        }
        change_.rule_data.emplace_back(std::move(data_id), std::move(id));
    }

    // NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
    void emplace_exclusion_data(std::string data_id, std::string id, data_spec spec) {
        change_.content = change_.content | change_set::exclusion_data;
        config_.content = config_.content | change_set::exclusion_data;

        auto it = config_.exclusion_data.find(data_id);
        if (it == config_.exclusion_data.end()) {
            auto &merged_spec = config_.exclusion_data[data_id];
            merged_spec.type = spec.type;
            merged_spec.values.emplace(id, std::move(spec.values));
        } else {
            // TODO fail if type differs
