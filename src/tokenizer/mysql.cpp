...
constexpr std::string_view identifier_regex_str =
    R"((?i)^(?:(?P<command>SELECT|ALL|DISTINCT|DISTINCTROW|HIGH_PRIORITY|STRAIGHT_JOIN|SQL_SMALL_RESULT|SQL_BIG_RESULT|SQL_BUFFER_RESULT|SQL_NO_CACHE|SQL_CALC_FOUND_ROWS|FROM|PARTITION|WHERE|GROUP\s+BY|WITH\s+ROLLUP|UNION\s+ALL|UNION|INTERSECT|EXCEPT|HAVING|WINDOW|ORDER\s+BY|ASC|DESC|LIMIT|OFFSET|AS)|(?P<binary_operator>MOD|AND|BETWEEN|BINARY|CASE|DIV|NOT\s+NULL|IS\s+NULL|IS\s+NOT\s+NULL|IS\s+NOT|IS|LAST_DAY|NOT\s+BETWEEN|NOT\s+LIKE|NOT\s+REGEXP|NOT\s+IN|NOT|REGEXP|XOR|OR|RLIKE|SOUNDS\s+LIKE|LIKE|IN)|(?P<identifier>[\x{0080}-\x{FFFF}a-zA-Z_][\x{0080}-\x{FFFF}a-zA-Z_0-9$]*))(?:\b|\s|$))";
