...
public:
    ...
    string_wrapper(char *str, std::size_t length, std::size_t allocated_length, std::pmr::memory_resource *alloc = std::pmr::new_delete_resource()): str_(str), length_(length), allocated_length_(allocated_length), alloc_(alloc) {}

    string_wrapper(const string_wrapper&) = delete;
    string_wrapper& operator=(const string_wrapper&) = delete;

    string_wrapper(string_wrapper &&other) noexcept : str_(other.str_), length_(other.length_), allocated_length_(other.allocated_length_), alloc_(other.alloc_) {
        other.str_ = nullptr;
        other.length_ = 0;
