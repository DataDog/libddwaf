...
public:
    ...
    explicit match_iterator(ResourceType resource, const object_view &obj,
        ...
        : resource_(resource), it_(obj, {}, exclude, limits)
    {
        for (; it_; ++it_) {
            ...
            if (current_obj.type() == object_type::string && current_obj->size() >= MinLength) {
                current_param_ = current_obj->template as_unchecked<
                    std::string_view>(); //{
    ...
    bool operator++()
    {
        ...
        while (++it_) {
            auto current_obj = *it_;
            if (current_obj.type() == object_type::string && current_obj->size() >= MinLength) {
                current_param_ = current_obj->template as_unchecked<std::string_view>();
                current_index_ = resource_.find(current_param_, 0);
                if (current_index_ != npos) {
                    return true;
                }
