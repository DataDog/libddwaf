...
uint32_t fetch_next_codepoint(const char * utf8Buffer, uint64_t& position, uint64_t length)
{
	...
	for (uint8_t byteIndex = 1; byteIndex < nextGlyphLength; ++byteIndex)
	{
		...
		// The bytes after the header are formatted like 10xxxxxx, we thus mask by 00111111 to only keep xxxxxx and append it at the end of codepoint
...
struct ScratchpadChunck
{
	...
	ScratchpadChunck(ScratchpadChunck&& chunck) : scratchpad(chunck.scratchpad), length(chunck.length), used(chunck.used)
	{
...
size_t normalize_codepoint(uint32_t codepoint, int32_t* wbBuffer, size_t wbBufferLength)
{
	...
	size_t decomposedLength = (size_t) utf8proc_decompose_char((int32_t) codepoint,
															   wbBuffer,
															   (utf8proc_ssize_t) wbBufferLength,
															   (utf8proc_option_t) (UTF8PROC_DECOMPOSE | UTF8PROC_IGNORE | UTF8PROC_COMPAT | UTF8PROC_LUMP | UTF8PROC_STRIPMARK | UTF8PROC_STRIPNA | UTF8PROC_CASEFOLD),
															   NULL);
...
bool normalize_string(char ** _utf8Buffer, uint64_t & bufferLength)
{
	...
	// A tricky part of this conversion is that the output size is totally unknown, but we want to be efficient with our allocations.
	// We're going to write the glyph we're normalising in a static buffer (if possible) and write the filtered, normalized results in a bunch of semi-fixed buffer (the scratchpad)
	// Only when the conversion is over will we allocate the final buffer and copy everything in there.
