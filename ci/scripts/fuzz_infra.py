...
def upload_corpus(pkgname, binary) -> bool:
    has_errors = False
    corpus_files_path = search_corpus(binary.replace("_fuzz", ""))
...
def build_all():
    ...
    os.system("/workspace/fuzzer/docker/build.sh")
    print("✅ Built all fuzzers")

def get_package_name(binary):
    return "libddwaf-" + binary.replace("_fuzz", "")[:MAX_PKG_NAME_LENGTH].replace("_", "-") # limit to 50 chars
    ...
    if os.path.isdir(directory):
        for fname in os.listdir(directory):
            ...
            if not is_executable(fpath):
                print(f"Skipping {fname} because it's not executable")
                continue
            if not fname.endswith("_fuzz"):
                print(f"Skipping {fname} because it doesn't end with _fuzz")
                continue
            print(f"Adding {fname} to list of fuzzers")
